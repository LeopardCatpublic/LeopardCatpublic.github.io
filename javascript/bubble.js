var scroller = document.querySelector('#scroller');
        var row = 10;
        var col = 16;
        var particle_ary_element_string = '';
        for (let i = 0; i < row; i++) {
            for (let j = 0; j < col; j++) {
                var random_number = Math.floor((Math.random() * 19) + 1);
                particle_ary_element_string += ` 
                    <div data-left="${(j + 1) * 175}" data-top="${(i + 1) * 175}" style="left: ${(j + 1) * 175}px; top: ${(i + 1) * 175}px" class="particle">
                        <div style="background-image: url('./circle-img/${random_number}.png')" data-cat="${random_number}"  class="particle__inner"></div>
                    </div>`;
            }
        }
        scroller.innerHTML += particle_ary_element_string;
var main = {
    init: function (o) {
    ...
    closeEl: function () {
      ...
      //
      var cat_id = $(this.currentEl).find(".particle__inner").data("cat");
     
      
      $(this.currentEl).find(".particle__inner").css('background-color','transparent');
      $(this.currentEl).find(".particle__inner").css('background-image',`url('../circle-img/${ cat_id }.png')`);
    ...
    showOnEl: function (el) {
      ...
      var circle = $(el).find(".particle__inner");
      $(circle).css('background-image','none');
      $(circle).css('background-color','black');

      var cat_id =$(circle).data("cat");
      $('#cat-image').attr('src',`../origin-img/${ cat_id }.jpg`);
